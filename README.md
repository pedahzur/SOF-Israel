...
Use the [interactive appendix](https://pedahzur.github.io/SOF-APPENDIX/) to browse, search, and filter 26 catalogued datasets and methodological documents. The register groups material into units, people, operations, terrorism, losses, and documentation, and identifies the preferred version when related files overlap.

The repository contains CSV and XLSX data plus methodological documentation. Logical CSV record counts are generated from the files themselves and published in [`record-counts.json`](record-counts.json); this avoids treating quoted line breaks as extra records.
...
The most comprehensive and up-to-date units database (March 2026), covering identified Israeli SOF and elite units across all eras. Fields include Unit, Affiliation, and Audio-video.

**Units_March_20262.csv**
Updated units database (March 2026) with organizational lineage tracking. Includes predecessor/successor relationships and affiliation details. Fields include Unit, Year Established, Affiliation, Predecessor, and Successor.

**UNITSN_194.csv**
Earlier version of the units database, including Hebrew-language cells and some missing values. Tracks unit lineage, organizational affiliation, and role. Fields include Name, Notes (Hebrew), Attachments, Predecessor, Successor, Affiliation, Year Established, Year Disbanded, Branch, Command, Rrole (Role), References, and Bibliography Attachment.
...
**UNITS-Grid view.csv**
Airtable grid-view export containing bilingual data (English names and Hebrew notes). Includes status classification and key individuals associated with each unit. Fields include ID, Name copy, Notes (Hebrew), Status (Conventional/Elite), Year Established, Year Disbanded, and Individuals.

**Units Glossary-Grid.csv**
Glossary-style reference mapping English unit names to Hebrew equivalents, with associated individuals and predecessor information. Fields include Unit1, UNIT - HEBREW, Individuals, and Predecessor.
...
Database of regional defensive and reconnaissance units with geographic coordinates, useful for spatial analysis of unit deployment. Fields include Name, Notes (year), Attachments, Location, Latitude, Longitude, REGIONAL COMMAND, Affiliation, and Predecessor.
...
Global reference list of notable individuals relevant to the study of special operations worldwide. Includes birth/death dates and Wikipedia links for reference. Fields include Name, Born, Died, and Link (Wikipedia URLs).
...
Database of operations during the War of Attrition period (1967-1970). Documents military operations with participating units, branch, scale, and location. Fields include DATE, NAME, Number of Units, Units, Branch (Land/Navy), SCALE (Small/Medium/Large), and Location.
...
**reprisals.csv**
Dataset documenting reprisal and military operations from 1952 to 1964. Includes operation names, dates, locations, and detailed place descriptions. Fields include Date, Date - Translated, Operation Name, Location, and Place.
...
After changing a CSV file, regenerate the public counts:

```bash
python3 scripts/generate_record_counts.py
python3 scripts/generate_record_counts.py --check
```

The generator uses a CSV parser rather than physical line counts, so quoted multiline fields are counted correctly. Both `record-counts.json` and the browser-readable `js/record-counts.js` are generated artifacts.

### Rights and Access

Not every research source may be redistributed. The GTD source data and codebook are intentionally absent from the public branch; obtain them directly from START. See [`RIGHTS_AND_ACCESS.md`](RIGHTS_AND_ACCESS.md) for the access links and repository policy.

After deploying a release, run `python3 scripts/check_live_rights.py` to confirm that the restricted paths return HTTP 404 or 410.

### Draft Material

[`Methodological_Appendix_Fallen_Oct7.md`](Methodological_Appendix_Fallen_Oct7.md) is a draft with unresolved verification markers. Its underlying record-level dataset is not included here, so its figures should not yet be cited as final.
